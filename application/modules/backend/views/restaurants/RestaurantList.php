...
<div id="page-wrapper">
	<div class="row">
		...
		<div class="col-lg-12">
			...
			<a href="<?php echo base_url();?>admin/restaurant/new" class="btn btn-primary view-contacts bottom-margin" >
                       	...
                       	<table class="table table-striped table-bordered table-hover" id="tblRestos">
							...
							<?php foreach ($restaurants as $item):?>
								<tr>
									...
									<td><a href = "<?php echo base_url();?>admin/restaurant/edit/<?php echo $item['id']?>" class="btn btn-success icon-btn btn-xs"><i class="fa fa-pencil"></i></a></td>
								</tr>
								<?php endforeach;?>
							<?php } else { ?>
...
function status()
{
	if($('#comment').val()!='')
	{
		...
		{
	$.get(base_url + "admin/restaurant/turnoffresto",{id: $('#restaurantid').val(),status:  $('#statusvalue').val(),comment: $('#comment').val()}, function (data) {
		...
		{
			$.get(base_url + "admin/restaurant/turnonresto",{id: $('#restaurantid').val(),status:  $('#statusvalue').val(),comment: $('#comment').val()}, function (data) {
			       alert('Status is changed');
			       location.reload();
		  	});
		}
...
$("#status").change(function() {
	window.location.href = base_url+"admin/restaurant/list?status="+$(this).val();
});
</script>